...
function Home() {
  ...
  useEffect (() => {
    fetch('https://my-moviz-backend-y49b.vercel.app/movies')
  ...
  // console.log(truncate("dddddddddddddddddddddddd",3))
  ...
  const movies = movieData.map((data, i) => {
    const isLiked = likedMovies.some(movie => movie === data.title);
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={truncate(data.overview, 250)} poster={url + data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
